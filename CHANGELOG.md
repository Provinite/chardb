...
- **`scripts/lib/remote-host.sh`**: shared SSH transport for the staging host, sourced by both `deploy.sh` and `scripts/ssh-dev.sh`. Set `DEPLOY_TRANSPORT=direct` to bypass the tunnel and connect straight to the Elastic IP — the break-glass route if the SSM agent is unhealthy.
...
- **`scripts/get-terraform-outputs.sh` masks its secrets under GitHub Actions** and creates `~/.ssh` before writing the key there. Callers consume the script through `source <(... | grep "^export")`, so the export lines are normally swallowed by the pipe — but on a public repository one step that ran it without capturing stdout would publish live credentials, so it now emits `::add-mask::` for every sensitive output instead of relying on each caller to remember. It also exports `INSTANCE_ID`, which the Session Manager tunnel addresses the host by.
