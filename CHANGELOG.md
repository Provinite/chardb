...
- **The staging host fetches its own OAuth secrets at deploy time.** `deploy.sh` no longer writes the DeviantArt, Discord and ToyHouse client ids, client secrets or bot token into the `.env` it uploads; the deploy script running on the host reads them from Parameter Store under `/chardb/dev/` using its instance role. It fails the deploy if any parameter is missing or still holds the `not-managed-by-terraform` placeholder, rather than booting an app that would reject every OAuth callback. They are still written into `.env` on the host, so `docker compose up -d` run by hand during a recovery keeps working — without them in the file Compose resolves `${DISCORD_BOT_TOKEN}` to an empty string with only a warning, silently recreating the backend with blank OAuth config. Values come back from `jq` base64-encoded so they survive the pipe intact, then get escaped for Compose's env-file parser: `\` → `\\`, `"` → `\"`, `$` → `$$`. Single-quoting instead would be simpler but makes Compose reject the entire file if a secret contains an apostrophe — verified against real Compose, as is the round trip for all seven live values. Prod needs no equivalent, since ECS already resolves Parameter Store ARNs through the task definition's `valueFrom`.
