...
export type PermissionKey = {
  [K in keyof Role]-?: K extends `can${string}`
    ? NonNullable<Role[K]> extends boolean
      ? K
      : never
    : never;
}[keyof Role];
...
export const PERMISSIONS_MAP = {
  ...
  canCreateOrphanedCharacter: {
    ...
    description:
      "Allow creation of characters without an owner (community/orphaned characters)",
    category: "content",
  },
  canEditCharacter: {
    name: "Edit Any Character (Profile)",
    description:
      "Allow editing profile fields (name, bio, visibility, trade settings) on any character",
    category: "content",
  },
  canEditOwnCharacter: {
    name: "Edit Own Characters (Profile)",
    description:
      "Allow editing profile fields (name, bio, visibility, trade settings) on owned characters",
    category: "content",
  },
  canEditCharacterRegistry: {
    name: "Edit Any Character (Registry)",
    description:
      "Allow editing registry fields (registry ID, variant, traits) on any character",
    category: "content",
  },
  canEditOwnCharacterRegistry: {
    name: "Edit Own Character (Registry)",
    description:
      "Allow editing registry fields (registry ID, variant, traits) on owned characters",
    category: "content",
  },
  canManageItems: {
    name: "Manage Items",
  ...
  canModerateImages: {
    ...
    description:
      "Allow reviewing and approving/rejecting uploaded images in the community",
    category: "content",
  },
  canDeleteCharacter: {
...
export const getPermissionsByCategory = (
  category: PermissionCategory,
): Permission[] => ALL_PERMISSIONS.filter((p) => p.category === category);
