...
export type Character = {
  ...
  /** The source of this character's pending trait review, or null when there is none. Says what kind of review is open without exposing what it proposes. */
  pendingTraitReviewSource: Maybe<TraitReviewSource>;
...
export type CreateTradeInput = {
  ...
  /** Characters the proposer hands over. Each must be theirs, open to trades, and in this community by way of its species. */
  offeringCharacters?: Array<TradeCharacterInput>;
  recipientId: Scalars['ID']['input'];
  requesting?: Array<RequestedTradeItemInput>;
  /** Characters the proposer asks for. Each must be the recipient's, and open to trades -- the flag is the owner's standing answer to being asked, and it is honoured again at settlement. */
  requestingCharacters?: Array<TradeCharacterInput>;
};
...
export type ItemUseMyoGrant = {
  __typename?: 'ItemUseMyoGrant';
  id: Scalars['ID']['output'];
  species: Species;
  /** Which variants a ticket of this type can make. Never empty -- a grant with no variants is cleared rather than stored. */
...
export type ItemUseTraitEditGrant = {
  __typename?: 'ItemUseTraitEditGrant';
  id: Scalars['ID']['output'];
  /** Never empty -- a grant covering nothing is cleared instead. */
  species: Array<ItemUseTraitEditGrantSpecies>;
};

export type ItemUseTraitEditGrantSpecies = {
  __typename?: 'ItemUseTraitEditGrantSpecies';
  id: Scalars['ID']['output'];
  species: Species;
...
/** Every item of one type that a member holds. */
export type MemberHolding = {
  __typename?: 'MemberHolding';
  count: Scalars['Int']['output'];
  itemType: ItemType;
  /** The items themselves, oldest first. Each has its own provenance, so they are listed rather than only counted. */
  items: Array<Item>;
};

/** One member's holdings in one community, with what they are still owed. */
export type MemberHoldingsReport = {
  __typename?: 'MemberHoldingsReport';
  /** Distinct item types held. */
  distinctTypes: Scalars['Int']['output'];
  /** Largest holding first. */
  holdings: Array<MemberHolding>;
  member: User;
  /** Items granted to an external account this member has linked, but not yet claimed. Always 0 today -- claiming happens automatically on link -- and present so the page can say so rather than stay silent. */
  pendingItems: Scalars['Int']['output'];
  /** Live items held, across every type. */
...
export type Mutation = {
  ...
  /** Spend an edit kit to propose a change to one of your characters' traits. Destroys the kit and creates a pending USER_EDIT review; the character is unchanged until staff approve it. Authorized by holding the kit, not by canEditOwnCharacterRegistry. */
  editCharacterTraitsWithKit: TraitReview;
  ...
  setItemTypeMyoGrant: ItemType;
  /** Set which characters an edit kit of this type can change the traits of. Replaces the grant wholesale; an empty species list clears it. A species with no variants listed covers every variant of it. */
...
export type MutationCreateCharacterFromMyoTicketArgs = {
...
export type MutationEditCharacterTraitsWithKitArgs = {
...
export type MutationSetItemTypeMyoGrantArgs = {
  input: SetItemTypeMyoGrantInput;
};


export type MutationSetItemTypeTraitEditGrantArgs = {
...
export type Query = {
  ...
  /** Every member's purchases, newest first, for staff handling a refund past the buyer's own undo window. */
  communityShopPurchases: Array<ShopPurchase>;
...
export type QueryCommunityShopPurchasesArgs = {
...
export type RedeemMyoTicketInput = {
...
export type SetItemTypeMyoGrantInput = {
  itemTypeId: Scalars['ID']['input'];
  /** The species a ticket makes. Required unless the variant list is empty, which clears the grant. */
  speciesId?: InputMaybe<Scalars['ID']['input']>;
  /** Replaces the grant wholesale. Empty clears it. */
  speciesVariantIds: Array<Scalars['ID']['input']>;
};

export type SetItemTypeTraitEditGrantInput = {
  itemTypeId: Scalars['ID']['input'];
  /** Replaces the grant wholesale. Empty clears it. */
  species: Array<TraitEditGrantSpeciesInput>;
};
...
export type ShopPurchase = {
  ...
  /** Who bought it. What a staff view is looking at. */
  buyer: Maybe<User>;
...
export type Trade = {
  __typename?: 'Trade';
  characterLines: Array<TradeCharacter>;
  community: Community;
  createdAt: Scalars['DateTime']['output'];
  ...
  /** Shared by every ledger row this settlement wrote, across the item ledger, the currency ledger and the character ownership log. Null until accepted. */
...
/** One character on the table, moving one way. Always a specific character: there is one of each, so there is no by-type equivalent of the item line. */
export type TradeCharacter = {
  __typename?: 'TradeCharacter';
  character: Character;
  /** Who receives them. */
...
/** A character on the table. One shape for both directions, unlike items: an item line can ask for any copies of a type, and a character has no copies -- there is one of it, and it is the one you meant. Which side it is on is the field it arrives in. */
export type TradeCharacterInput = {
  characterId: Scalars['ID']['input'];
};
...
export type Trait = {
  __typename?: 'Trait';
  /** Whether this trait allows an optional free-text clarifier on each value */
  allowsClarifier: Scalars['Boolean']['output'];
  /** Whether this trait allows multiple values per character */
...
export type TraitEditGrantSpeciesInput = {
  speciesId: Scalars['ID']['input'];
  /** Narrow the kit to these variants. Empty covers every variant of the species, and characters with no variant set. */
  speciesVariantIds?: Array<Scalars['ID']['input']>;
};
...
export type GetItemTransactionsQuery = { __typename?: 'Query', itemTransactions: { __typename?: 'ItemTransactionConnection', total: number, hasMore: boolean, transactions: Array<{ __typename?: 'ItemTransaction', id: string, communityId: string, kind: ItemTransactionKind, batchId: string, batchSize: number, reason: string | null, staffNote: string | null, actorLabel: string | null, createdAt: string, itemId: string, itemType: { __typename?: 'ItemType', id: string, name: string, category: string | null, color: { __typename?: 'CommunityColor', id: string, hexCode: string } | null, image: { __typename?: 'Image', id: string, thumbnailUrl: string | null, originalUrl: string, altText: string | null } | null }, fromUser: { __typename?: 'User', id: string, username: string, displayName: string | null, avatarImage: { __typename?: 'Image', id: string, originalUrl: string, thumbnailUrl: string | null, altText: string | null } | null } | null, toUser: { __typename?: 'User', id: string, username: string, displayName: string | null, avatarImage: { __typename?: 'Image', id: string, originalUrl: string, thumbnailUrl: string | null, altText: string | null } | null } | null, actorUser: { __typename?: 'User', id: string, username: string, displayName: string | null, avatarImage: { __typename?: 'Image', id: string, originalUrl: string, thumbnailUrl: string | null, altText: string | null } | null } | null }> } };
...
export type GetMyoTicketQueryVariables = Exact<{
  itemId: Scalars['ID']['input'];
}>;
...
export type CreateCharacterFromMyoTicketMutation = { __typename?: 'Mutation', createCharacterFromMyoTicket: { __typename?: 'Character', id: string, name: string } };

export type SetItemTypeMyoGrantMutationVariables = Exact<{
  input: SetItemTypeMyoGrantInput;
}>;


export type SetItemTypeMyoGrantMutation = { __typename?: 'Mutation', setItemTypeMyoGrant: { __typename?: 'ItemType', id: string, name: string, description: string | null, communityId: string, category: string | null, isTradeable: boolean, isConsumable: boolean, colorId: string | null, metadata: any | null, createdAt: string, updatedAt: string, usePayout: Array<{ __typename?: 'ItemUsePayoutComponent', id: string, amount: number, currency: { __typename?: 'Currency', id: string, name: string, code: string, symbol: string | null } }>, useMyoGrant: { __typename?: 'ItemUseMyoGrant', id: string, species: { __typename?: 'Species', id: string, name: string }, variants: Array<{ __typename?: 'SpeciesVariant', id: string, name: string }> } | null, useTraitEditGrant: { __typename?: 'ItemUseTraitEditGrant', id: string, species: Array<{ __typename?: 'ItemUseTraitEditGrantSpecies', id: string, species: { __typename?: 'Species', id: string, name: string }, variants: Array<{ __typename?: 'SpeciesVariant', id: string, name: string }> }> } | null, useVariantChangeGrant: { __typename?: 'ItemUseVariantChangeGrant', id: string, species: { __typename?: 'Species', id: string, name: string }, toVariant: { __typename?: 'SpeciesVariant', id: string, name: string }, fromVariants: Array<{ __typename?: 'SpeciesVariant', id: string, name: string }> } | null, image: { __typename?: 'Image', id: string, originalUrl: string, thumbnailUrl: string | null, altText: string | null } | null, color: { __typename?: 'CommunityColor', id: string, name: string, hexCode: string } | null } };

export type SetItemTypeTraitEditGrantMutationVariables = Exact<{
  input: SetItemTypeTraitEditGrantInput;
}>;


export type SetItemTypeTraitEditGrantMutation = { __typename?: 'Mutation', setItemTypeTraitEditGrant: { __typename?: 'ItemType', id: string, name: string, description: string | null, communityId: string, category: string | null, isTradeable: boolean, isConsumable: boolean, colorId: string | null, metadata: any | null, createdAt: string, updatedAt: string, usePayout: Array<{ __typename?: 'ItemUsePayoutComponent', id: string, amount: number, currency: { __typename?: 'Currency', id: string, name: string, code: string, symbol: string | null } }>, useMyoGrant: { __typename?: 'ItemUseMyoGrant', id: string, species: { __typename?: 'Species', id: string, name: string }, variants: Array<{ __typename?: 'SpeciesVariant', id: string, name: string }> } | null, useTraitEditGrant: { __typename?: 'ItemUseTraitEditGrant', id: string, species: Array<{ __typename?: 'ItemUseTraitEditGrantSpecies', id: string, species: { __typename?: 'Species', id: string, name: string }, variants: Array<{ __typename?: 'SpeciesVariant', id: string, name: string }> }> } | null, useVariantChangeGrant: { __typename?: 'ItemUseVariantChangeGrant', id: string, species: { __typename?: 'Species', id: string, name: string }, toVariant: { __typename?: 'SpeciesVariant', id: string, name: string }, fromVariants: Array<{ __typename?: 'SpeciesVariant', id: string, name: string }> } | null, image: { __typename?: 'Image', id: string, originalUrl: string, thumbnailUrl: string | null, altText: string | null } | null, color: { __typename?: 'CommunityColor', id: string, name: string, hexCode: string } | null } };

export type GetEditKitQueryVariables = Exact<{
  itemId: Scalars['ID']['input'];
}>;
...
export type GetMyCharactersForEditKitQuery = { __typename?: 'Query', myCharacters: { __typename?: 'CharacterConnection', total: number, hasMore: boolean, characters: Array<{ __typename?: 'Character', id: string, name: string, speciesId: string | null, speciesVariantId: string | null, traitReviewStatus: ModerationStatus | null, species: { __typename?: 'Species', id: string, name: string } | null, speciesVariant: { __typename?: 'SpeciesVariant', id: string, name: string } | null }> } };

export type EditCharacterTraitsWithKitMutationVariables = Exact<{
  input: EditCharacterTraitsWithKitInput;
}>;


export type EditCharacterTraitsWithKitMutation = { __typename?: 'Mutation', editCharacterTraitsWithKit: { __typename?: 'TraitReview', id: string, status: ModerationStatus, characterId: string } };

export type SetItemTypeVariantChangeGrantMutationVariables = Exact<{
  input: SetItemTypeVariantChangeGrantInput;
}>;


export type SetItemTypeVariantChangeGrantMutation = { __typename?: 'Mutation', setItemTypeVariantChangeGrant: { __typename?: 'ItemType', id: string, name: string, description: string | null, communityId: string, category: string | null, isTradeable: boolean, isConsumable: boolean, colorId: string | null, metadata: any | null, createdAt: string, updatedAt: string, usePayout: Array<{ __typename?: 'ItemUsePayoutComponent', id: string, amount: number, currency: { __typename?: 'Currency', id: string, name: string, code: string, symbol: string | null } }>, useMyoGrant: { __typename?: 'ItemUseMyoGrant', id: string, species: { __typename?: 'Species', id: string, name: string }, variants: Array<{ __typename?: 'SpeciesVariant', id: string, name: string }> } | null, useTraitEditGrant: { __typename?: 'ItemUseTraitEditGrant', id: string, species: Array<{ __typename?: 'ItemUseTraitEditGrantSpecies', id: string, species: { __typename?: 'Species', id: string, name: string }, variants: Array<{ __typename?: 'SpeciesVariant', id: string, name: string }> }> } | null, useVariantChangeGrant: { __typename?: 'ItemUseVariantChangeGrant', id: string, species: { __typename?: 'Species', id: string, name: string }, toVariant: { __typename?: 'SpeciesVariant', id: string, name: string }, fromVariants: Array<{ __typename?: 'SpeciesVariant', id: string, name: string }> } | null, image: { __typename?: 'Image', id: string, originalUrl: string, thumbnailUrl: string | null, altText: string | null } | null, color: { __typename?: 'CommunityColor', id: string, name: string, hexCode: string } | null } };

export type GetVariantChangeItemQueryVariables = Exact<{
  itemId: Scalars['ID']['input'];
}>;
...
export type ChangeCharacterVariantWithItemMutation = { __typename?: 'Mutation', changeCharacterVariantWithItem: { __typename?: 'Character', id: string, name: string, speciesVariantId: string | null } };
...
export type TraitDetailsFragment = { __typename?: 'Trait', id: string, name: string, valueType: TraitValueType, allowsMultipleValues: boolean, allowsClarifier: boolean, speciesId: string, colorId: string | null, createdAt: string, updatedAt: string, color: { __typename?: 'CommunityColor', id: string, name: string, hexCode: string } | null };

export type TraitConnectionDetailsFragment = { __typename?: 'TraitConnection', hasNextPage: boolean, hasPreviousPage: boolean, totalCount: number, nodes: Array<{ __typename?: 'Trait', id: string, name: string, valueType: TraitValueType, allowsMultipleValues: boolean, allowsClarifier: boolean, speciesId: string, colorId: string | null, createdAt: string, updatedAt: string, color: { __typename?: 'CommunityColor', id: string, name: string, hexCode: string } | null }> };
...
export const UpdateCharacterRegistryDocument = gql`
...
/**
 * __useUpdateCharacterRegistryMutation__
 *
 * To run a mutation, you first call `useUpdateCharacterRegistryMutation` within a React component and pass it any options that fit your needs.
 * When your component renders, `useUpdateCharacterRegistryMutation` returns a tuple that includes:
 * - A mutate function that you can call at any time to execute the mutation
 * - An object with fields that represent the current status of the mutation's execution
 *
 * @param baseOptions options that will be passed into the mutation, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options-2;
 *
 * @example
 * const [updateCharacterRegistryMutation, { data, loading, error }] = useUpdateCharacterRegistryMutation({
 *   variables: {
 *      id: // value for 'id'
 *      input: // value for 'input'
 *   },
 * });
 */
export function useUpdateCharacterRegistryMutation(baseOptions?: Apollo.MutationHookOptions<UpdateCharacterRegistryMutation, UpdateCharacterRegistryMutationVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useMutation<UpdateCharacterRegistryMutation, UpdateCharacterRegistryMutationVariables>(UpdateCharacterRegistryDocument, options);
      }
export type UpdateCharacterRegistryMutationHookResult = ReturnType<typeof useUpdateCharacterRegistryMutation>;
export type UpdateCharacterRegistryMutationResult = Apollo.MutationResult<UpdateCharacterRegistryMutation>;
export type UpdateCharacterRegistryMutationOptions = Apollo.BaseMutationOptions<UpdateCharacterRegistryMutation, UpdateCharacterRegistryMutationVariables>;
...
export const GetMemberHoldingsDocument = gql`
...
/**
 * __useGetMemberHoldingsQuery__
 *
 * To run a query within a React component, call `useGetMemberHoldingsQuery` and pass it any options that fit your needs.
 * When your component renders, `useGetMemberHoldingsQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useGetMemberHoldingsQuery({
 *   variables: {
 *      communityId: // value for 'communityId'
 *      userId: // value for 'userId'
 *   },
 * });
 */
export function useGetMemberHoldingsQuery(baseOptions: Apollo.QueryHookOptions<GetMemberHoldingsQuery, GetMemberHoldingsQueryVariables> & ({ variables: GetMemberHoldingsQueryVariables; skip?: boolean; } | { skip: boolean; }) ) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<GetMemberHoldingsQuery, GetMemberHoldingsQueryVariables>(GetMemberHoldingsDocument, options);
      }
export function useGetMemberHoldingsLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<GetMemberHoldingsQuery, GetMemberHoldingsQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<GetMemberHoldingsQuery, GetMemberHoldingsQueryVariables>(GetMemberHoldingsDocument, options);
        }
export function useGetMemberHoldingsSuspenseQuery(baseOptions?: Apollo.SkipToken | Apollo.SuspenseQueryHookOptions<GetMemberHoldingsQuery, GetMemberHoldingsQueryVariables>) {
          const options = baseOptions === Apollo.skipToken ? baseOptions : {...defaultOptions, ...baseOptions}
          return Apollo.useSuspenseQuery<GetMemberHoldingsQuery, GetMemberHoldingsQueryVariables>(GetMemberHoldingsDocument, options);
        }
export type GetMemberHoldingsQueryHookResult = ReturnType<typeof useGetMemberHoldingsQuery>;
...
export const GetMyoTicketDocument = gql`
...
/**
 * __useGetMyoTicketQuery__
 *
 * To run a query within a React component, call `useGetMyoTicketQuery` and pass it any options that fit your needs.
 * When your component renders, `useGetMyoTicketQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useGetMyoTicketQuery({
 *   variables: {
 *      itemId: // value for 'itemId'
 *   },
 * });
 */
export function useGetMyoTicketQuery(baseOptions: Apollo.QueryHookOptions<GetMyoTicketQuery, GetMyoTicketQueryVariables> & ({ variables: GetMyoTicketQueryVariables; skip?: boolean; } | { skip: boolean; }) ) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<GetMyoTicketQuery, GetMyoTicketQueryVariables>(GetMyoTicketDocument, options);
      }
export function useGetMyoTicketLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<GetMyoTicketQuery, GetMyoTicketQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<GetMyoTicketQuery, GetMyoTicketQueryVariables>(GetMyoTicketDocument, options);
        }
export function useGetMyoTicketSuspenseQuery(baseOptions?: Apollo.SkipToken | Apollo.SuspenseQueryHookOptions<GetMyoTicketQuery, GetMyoTicketQueryVariables>) {
          const options = baseOptions === Apollo.skipToken ? baseOptions : {...defaultOptions, ...baseOptions}
          return Apollo.useSuspenseQuery<GetMyoTicketQuery, GetMyoTicketQueryVariables>(GetMyoTicketDocument, options);
        }
export type GetMyoTicketQueryHookResult = ReturnType<typeof useGetMyoTicketQuery>;
export type GetMyoTicketLazyQueryHookResult = ReturnType<typeof useGetMyoTicketLazyQuery>;
export type GetMyoTicketSuspenseQueryHookResult = ReturnType<typeof useGetMyoTicketSuspenseQuery>;
export type GetMyoTicketQueryResult = Apollo.QueryResult<GetMyoTicketQuery, GetMyoTicketQueryVariables>;
export const CreateCharacterFromMyoTicketDocument = gql`
  ...
  createCharacterFromMyoTicket(input: $input) {
    ...
    `;
export type CreateCharacterFromMyoTicketMutationFn = Apollo.MutationFunction<CreateCharacterFromMyoTicketMutation, CreateCharacterFromMyoTicketMutationVariables>;

/**
 * __useCreateCharacterFromMyoTicketMutation__
 *
 * To run a mutation, you first call `useCreateCharacterFromMyoTicketMutation` within a React component and pass it any options that fit your needs.
 * When your component renders, `useCreateCharacterFromMyoTicketMutation` returns a tuple that includes:
 * - A mutate function that you can call at any time to execute the mutation
 * - An object with fields that represent the current status of the mutation's execution
 *
 * @param baseOptions options that will be passed into the mutation, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options-2;
 *
 * @example
 * const [createCharacterFromMyoTicketMutation, { data, loading, error }] = useCreateCharacterFromMyoTicketMutation({
 *   variables: {
 *      input: // value for 'input'
 *   },
 * });
 */
export function useCreateCharacterFromMyoTicketMutation(baseOptions?: Apollo.MutationHookOptions<CreateCharacterFromMyoTicketMutation, CreateCharacterFromMyoTicketMutationVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useMutation<CreateCharacterFromMyoTicketMutation, CreateCharacterFromMyoTicketMutationVariables>(CreateCharacterFromMyoTicketDocument, options);
      }
export type CreateCharacterFromMyoTicketMutationHookResult = ReturnType<typeof useCreateCharacterFromMyoTicketMutation>;
export type CreateCharacterFromMyoTicketMutationResult = Apollo.MutationResult<CreateCharacterFromMyoTicketMutation>;
export type CreateCharacterFromMyoTicketMutationOptions = Apollo.BaseMutationOptions<CreateCharacterFromMyoTicketMutation, CreateCharacterFromMyoTicketMutationVariables>;
export const SetItemTypeMyoGrantDocument = gql`
  ...
  setItemTypeMyoGrant(input: $input) {
    ...
    ${ItemTypeFieldsFragmentDoc}`;
export type SetItemTypeMyoGrantMutationFn = Apollo.MutationFunction<SetItemTypeMyoGrantMutation, SetItemTypeMyoGrantMutationVariables>;

/**
 * __useSetItemTypeMyoGrantMutation__
 *
 * To run a mutation, you first call `useSetItemTypeMyoGrantMutation` within a React component and pass it any options that fit your needs.
 * When your component renders, `useSetItemTypeMyoGrantMutation` returns a tuple that includes:
 * - A mutate function that you can call at any time to execute the mutation
 * - An object with fields that represent the current status of the mutation's execution
 *
 * @param baseOptions options that will be passed into the mutation, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options-2;
 *
 * @example
 * const [setItemTypeMyoGrantMutation, { data, loading, error }] = useSetItemTypeMyoGrantMutation({
 *   variables: {
 *      input: // value for 'input'
 *   },
 * });
 */
export function useSetItemTypeMyoGrantMutation(baseOptions?: Apollo.MutationHookOptions<SetItemTypeMyoGrantMutation, SetItemTypeMyoGrantMutationVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useMutation<SetItemTypeMyoGrantMutation, SetItemTypeMyoGrantMutationVariables>(SetItemTypeMyoGrantDocument, options);
      }
export type SetItemTypeMyoGrantMutationHookResult = ReturnType<typeof useSetItemTypeMyoGrantMutation>;
export type SetItemTypeMyoGrantMutationResult = Apollo.MutationResult<SetItemTypeMyoGrantMutation>;
export type SetItemTypeMyoGrantMutationOptions = Apollo.BaseMutationOptions<SetItemTypeMyoGrantMutation, SetItemTypeMyoGrantMutationVariables>;
export const SetItemTypeTraitEditGrantDocument = gql`
  ...
  setItemTypeTraitEditGrant(input: $input) {
    ...
    ${ItemTypeFieldsFragmentDoc}`;
export type SetItemTypeTraitEditGrantMutationFn = Apollo.MutationFunction<SetItemTypeTraitEditGrantMutation, SetItemTypeTraitEditGrantMutationVariables>;

/**
 * __useSetItemTypeTraitEditGrantMutation__
 *
 * To run a mutation, you first call `useSetItemTypeTraitEditGrantMutation` within a React component and pass it any options that fit your needs.
 * When your component renders, `useSetItemTypeTraitEditGrantMutation` returns a tuple that includes:
 * - A mutate function that you can call at any time to execute the mutation
 * - An object with fields that represent the current status of the mutation's execution
 *
 * @param baseOptions options that will be passed into the mutation, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options-2;
 *
 * @example
 * const [setItemTypeTraitEditGrantMutation, { data, loading, error }] = useSetItemTypeTraitEditGrantMutation({
 *   variables: {
 *      input: // value for 'input'
 *   },
 * });
 */
export function useSetItemTypeTraitEditGrantMutation(baseOptions?: Apollo.MutationHookOptions<SetItemTypeTraitEditGrantMutation, SetItemTypeTraitEditGrantMutationVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useMutation<SetItemTypeTraitEditGrantMutation, SetItemTypeTraitEditGrantMutationVariables>(SetItemTypeTraitEditGrantDocument, options);
      }
export type SetItemTypeTraitEditGrantMutationHookResult = ReturnType<typeof useSetItemTypeTraitEditGrantMutation>;
export type SetItemTypeTraitEditGrantMutationResult = Apollo.MutationResult<SetItemTypeTraitEditGrantMutation>;
export type SetItemTypeTraitEditGrantMutationOptions = Apollo.BaseMutationOptions<SetItemTypeTraitEditGrantMutation, SetItemTypeTraitEditGrantMutationVariables>;
export const GetEditKitDocument = gql`
...
/**
 * __useGetEditKitQuery__
 *
 * To run a query within a React component, call `useGetEditKitQuery` and pass it any options that fit your needs.
 * When your component renders, `useGetEditKitQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useGetEditKitQuery({
 *   variables: {
 *      itemId: // value for 'itemId'
 *   },
 * });
 */
export function useGetEditKitQuery(baseOptions: Apollo.QueryHookOptions<GetEditKitQuery, GetEditKitQueryVariables> & ({ variables: GetEditKitQueryVariables; skip?: boolean; } | { skip: boolean; }) ) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<GetEditKitQuery, GetEditKitQueryVariables>(GetEditKitDocument, options);
      }
export function useGetEditKitLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<GetEditKitQuery, GetEditKitQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<GetEditKitQuery, GetEditKitQueryVariables>(GetEditKitDocument, options);
        }
export function useGetEditKitSuspenseQuery(baseOptions?: Apollo.SkipToken | Apollo.SuspenseQueryHookOptions<GetEditKitQuery, GetEditKitQueryVariables>) {
          const options = baseOptions === Apollo.skipToken ? baseOptions : {...defaultOptions, ...baseOptions}
          return Apollo.useSuspenseQuery<GetEditKitQuery, GetEditKitQueryVariables>(GetEditKitDocument, options);
        }
export type GetEditKitQueryHookResult = ReturnType<typeof useGetEditKitQuery>;
export type GetEditKitLazyQueryHookResult = ReturnType<typeof useGetEditKitLazyQuery>;
export type GetEditKitSuspenseQueryHookResult = ReturnType<typeof useGetEditKitSuspenseQuery>;
export type GetEditKitQueryResult = Apollo.QueryResult<GetEditKitQuery, GetEditKitQueryVariables>;
export const GetMyEditKitsDocument = gql`
...
/**
 * __useGetMyEditKitsQuery__
 *
 * To run a query within a React component, call `useGetMyEditKitsQuery` and pass it any options that fit your needs.
 * When your component renders, `useGetMyEditKitsQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useGetMyEditKitsQuery({
 *   variables: {
 *      communityId: // value for 'communityId'
 *      userId: // value for 'userId'
 *   },
 * });
 */
export function useGetMyEditKitsQuery(baseOptions: Apollo.QueryHookOptions<GetMyEditKitsQuery, GetMyEditKitsQueryVariables> & ({ variables: GetMyEditKitsQueryVariables; skip?: boolean; } | { skip: boolean; }) ) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<GetMyEditKitsQuery, GetMyEditKitsQueryVariables>(GetMyEditKitsDocument, options);
      }
export function useGetMyEditKitsLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<GetMyEditKitsQuery, GetMyEditKitsQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<GetMyEditKitsQuery, GetMyEditKitsQueryVariables>(GetMyEditKitsDocument, options);
        }
export function useGetMyEditKitsSuspenseQuery(baseOptions?: Apollo.SkipToken | Apollo.SuspenseQueryHookOptions<GetMyEditKitsQuery, GetMyEditKitsQueryVariables>) {
          const options = baseOptions === Apollo.skipToken ? baseOptions : {...defaultOptions, ...baseOptions}
          return Apollo.useSuspenseQuery<GetMyEditKitsQuery, GetMyEditKitsQueryVariables>(GetMyEditKitsDocument, options);
        }
export type GetMyEditKitsQueryHookResult = ReturnType<typeof useGetMyEditKitsQuery>;
export type GetMyEditKitsLazyQueryHookResult = ReturnType<typeof useGetMyEditKitsLazyQuery>;
export type GetMyEditKitsSuspenseQueryHookResult = ReturnType<typeof useGetMyEditKitsSuspenseQuery>;
export type GetMyEditKitsQueryResult = Apollo.QueryResult<GetMyEditKitsQuery, GetMyEditKitsQueryVariables>;
export const GetMyCharactersForEditKitDocument = gql`
...
/**
 * __useGetMyCharactersForEditKitQuery__
 *
 * To run a query within a React component, call `useGetMyCharactersForEditKitQuery` and pass it any options that fit your needs.
 * When your component renders, `useGetMyCharactersForEditKitQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useGetMyCharactersForEditKitQuery({
 *   variables: {
 *      filters: // value for 'filters'
 *   },
 * });
 */
export function useGetMyCharactersForEditKitQuery(baseOptions?: Apollo.QueryHookOptions<GetMyCharactersForEditKitQuery, GetMyCharactersForEditKitQueryVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<GetMyCharactersForEditKitQuery, GetMyCharactersForEditKitQueryVariables>(GetMyCharactersForEditKitDocument, options);
      }
export function useGetMyCharactersForEditKitLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<GetMyCharactersForEditKitQuery, GetMyCharactersForEditKitQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<GetMyCharactersForEditKitQuery, GetMyCharactersForEditKitQueryVariables>(GetMyCharactersForEditKitDocument, options);
        }
export function useGetMyCharactersForEditKitSuspenseQuery(baseOptions?: Apollo.SkipToken | Apollo.SuspenseQueryHookOptions<GetMyCharactersForEditKitQuery, GetMyCharactersForEditKitQueryVariables>) {
          const options = baseOptions === Apollo.skipToken ? baseOptions : {...defaultOptions, ...baseOptions}
          return Apollo.useSuspenseQuery<GetMyCharactersForEditKitQuery, GetMyCharactersForEditKitQueryVariables>(GetMyCharactersForEditKitDocument, options);
        }
export type GetMyCharactersForEditKitQueryHookResult = ReturnType<typeof useGetMyCharactersForEditKitQuery>;
export type GetMyCharactersForEditKitLazyQueryHookResult = ReturnType<typeof useGetMyCharactersForEditKitLazyQuery>;
export type GetMyCharactersForEditKitSuspenseQueryHookResult = ReturnType<typeof useGetMyCharactersForEditKitSuspenseQuery>;
export type GetMyCharactersForEditKitQueryResult = Apollo.QueryResult<GetMyCharactersForEditKitQuery, GetMyCharactersForEditKitQueryVariables>;
export const EditCharacterTraitsWithKitDocument = gql`
    mutation EditCharacterTraitsWithKit($input: EditCharacterTraitsWithKitInput!) {
  editCharacterTraitsWithKit(input: $input) {
    ...
    `;
export type EditCharacterTraitsWithKitMutationFn = Apollo.MutationFunction<EditCharacterTraitsWithKitMutation, EditCharacterTraitsWithKitMutationVariables>;

/**
 * __useEditCharacterTraitsWithKitMutation__
 *
 * To run a mutation, you first call `useEditCharacterTraitsWithKitMutation` within a React component and pass it any options that fit your needs.
 * When your component renders, `useEditCharacterTraitsWithKitMutation` returns a tuple that includes:
 * - A mutate function that you can call at any time to execute the mutation
 * - An object with fields that represent the current status of the mutation's execution
 *
 * @param baseOptions options that will be passed into the mutation, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options-2;
 *
 * @example
 * const [editCharacterTraitsWithKitMutation, { data, loading, error }] = useEditCharacterTraitsWithKitMutation({
 *   variables: {
 *      input: // value for 'input'
 *   },
 * });
 */
export function useEditCharacterTraitsWithKitMutation(baseOptions?: Apollo.MutationHookOptions<EditCharacterTraitsWithKitMutation, EditCharacterTraitsWithKitMutationVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useMutation<EditCharacterTraitsWithKitMutation, EditCharacterTraitsWithKitMutationVariables>(EditCharacterTraitsWithKitDocument, options);
      }
export type EditCharacterTraitsWithKitMutationHookResult = ReturnType<typeof useEditCharacterTraitsWithKitMutation>;
export type EditCharacterTraitsWithKitMutationResult = Apollo.MutationResult<EditCharacterTraitsWithKitMutation>;
export type EditCharacterTraitsWithKitMutationOptions = Apollo.BaseMutationOptions<EditCharacterTraitsWithKitMutation, EditCharacterTraitsWithKitMutationVariables>;
export const SetItemTypeVariantChangeGrantDocument = gql`
    mutation SetItemTypeVariantChangeGrant($input: SetItemTypeVariantChangeGrantInput!) {
  setItemTypeVariantChangeGrant(input: $input) {
    ...ItemTypeFields
  }
}
    ${ItemTypeFieldsFragmentDoc}`;
export type SetItemTypeVariantChangeGrantMutationFn = Apollo.MutationFunction<SetItemTypeVariantChangeGrantMutation, SetItemTypeVariantChangeGrantMutationVariables>;

/**
 * __useSetItemTypeVariantChangeGrantMutation__
 *
 * To run a mutation, you first call `useSetItemTypeVariantChangeGrantMutation` within a React component and pass it any options that fit your needs.
 * When your component renders, `useSetItemTypeVariantChangeGrantMutation` returns a tuple that includes:
 * - A mutate function that you can call at any time to execute the mutation
 * - An object with fields that represent the current status of the mutation's execution
 *
 * @param baseOptions options that will be passed into the mutation, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options-2;
 *
 * @example
 * const [setItemTypeVariantChangeGrantMutation, { data, loading, error }] = useSetItemTypeVariantChangeGrantMutation({
 *   variables: {
 *      input: // value for 'input'
 *   },
 * });
 */
export function useSetItemTypeVariantChangeGrantMutation(baseOptions?: Apollo.MutationHookOptions<SetItemTypeVariantChangeGrantMutation, SetItemTypeVariantChangeGrantMutationVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useMutation<SetItemTypeVariantChangeGrantMutation, SetItemTypeVariantChangeGrantMutationVariables>(SetItemTypeVariantChangeGrantDocument, options);
      }
export type SetItemTypeVariantChangeGrantMutationHookResult = ReturnType<typeof useSetItemTypeVariantChangeGrantMutation>;
export type SetItemTypeVariantChangeGrantMutationResult = Apollo.MutationResult<SetItemTypeVariantChangeGrantMutation>;
export type SetItemTypeVariantChangeGrantMutationOptions = Apollo.BaseMutationOptions<SetItemTypeVariantChangeGrantMutation, SetItemTypeVariantChangeGrantMutationVariables>;
export const GetVariantChangeItemDocument = gql`
...
/**
 * __useGetVariantChangeItemQuery__
 *
 * To run a query within a React component, call `useGetVariantChangeItemQuery` and pass it any options that fit your needs.
 * When your component renders, `useGetVariantChangeItemQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useGetVariantChangeItemQuery({
 *   variables: {
 *      itemId: // value for 'itemId'
 *   },
 * });
 */
export function useGetVariantChangeItemQuery(baseOptions: Apollo.QueryHookOptions<GetVariantChangeItemQuery, GetVariantChangeItemQueryVariables> & ({ variables: GetVariantChangeItemQueryVariables; skip?: boolean; } | { skip: boolean; }) ) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<GetVariantChangeItemQuery, GetVariantChangeItemQueryVariables>(GetVariantChangeItemDocument, options);
      }
export function useGetVariantChangeItemLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<GetVariantChangeItemQuery, GetVariantChangeItemQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<GetVariantChangeItemQuery, GetVariantChangeItemQueryVariables>(GetVariantChangeItemDocument, options);
        }
export function useGetVariantChangeItemSuspenseQuery(baseOptions?: Apollo.SkipToken | Apollo.SuspenseQueryHookOptions<GetVariantChangeItemQuery, GetVariantChangeItemQueryVariables>) {
          const options = baseOptions === Apollo.skipToken ? baseOptions : {...defaultOptions, ...baseOptions}
          return Apollo.useSuspenseQuery<GetVariantChangeItemQuery, GetVariantChangeItemQueryVariables>(GetVariantChangeItemDocument, options);
        }
export type GetVariantChangeItemQueryHookResult = ReturnType<typeof useGetVariantChangeItemQuery>;
export type GetVariantChangeItemLazyQueryHookResult = ReturnType<typeof useGetVariantChangeItemLazyQuery>;
export type GetVariantChangeItemSuspenseQueryHookResult = ReturnType<typeof useGetVariantChangeItemSuspenseQuery>;
export type GetVariantChangeItemQueryResult = Apollo.QueryResult<GetVariantChangeItemQuery, GetVariantChangeItemQueryVariables>;
export const GetMyVariantChangeItemsDocument = gql`
...
/**
 * __useGetMyVariantChangeItemsQuery__
 *
 * To run a query within a React component, call `useGetMyVariantChangeItemsQuery` and pass it any options that fit your needs.
 * When your component renders, `useGetMyVariantChangeItemsQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useGetMyVariantChangeItemsQuery({
 *   variables: {
 *      communityId: // value for 'communityId'
 *      userId: // value for 'userId'
 *   },
 * });
 */
export function useGetMyVariantChangeItemsQuery(baseOptions: Apollo.QueryHookOptions<GetMyVariantChangeItemsQuery, GetMyVariantChangeItemsQueryVariables> & ({ variables: GetMyVariantChangeItemsQueryVariables; skip?: boolean; } | { skip: boolean; }) ) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<GetMyVariantChangeItemsQuery, GetMyVariantChangeItemsQueryVariables>(GetMyVariantChangeItemsDocument, options);
      }
export function useGetMyVariantChangeItemsLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<GetMyVariantChangeItemsQuery, GetMyVariantChangeItemsQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<GetMyVariantChangeItemsQuery, GetMyVariantChangeItemsQueryVariables>(GetMyVariantChangeItemsDocument, options);
        }
export function useGetMyVariantChangeItemsSuspenseQuery(baseOptions?: Apollo.SkipToken | Apollo.SuspenseQueryHookOptions<GetMyVariantChangeItemsQuery, GetMyVariantChangeItemsQueryVariables>) {
          const options = baseOptions === Apollo.skipToken ? baseOptions : {...defaultOptions, ...baseOptions}
          return Apollo.useSuspenseQuery<GetMyVariantChangeItemsQuery, GetMyVariantChangeItemsQueryVariables>(GetMyVariantChangeItemsDocument, options);
        }
export type GetMyVariantChangeItemsQueryHookResult = ReturnType<typeof useGetMyVariantChangeItemsQuery>;
export type GetMyVariantChangeItemsLazyQueryHookResult = ReturnType<typeof useGetMyVariantChangeItemsLazyQuery>;
export type GetMyVariantChangeItemsSuspenseQueryHookResult = ReturnType<typeof useGetMyVariantChangeItemsSuspenseQuery>;
export type GetMyVariantChangeItemsQueryResult = Apollo.QueryResult<GetMyVariantChangeItemsQuery, GetMyVariantChangeItemsQueryVariables>;
export const ChangeCharacterVariantWithItemDocument = gql`
    mutation ChangeCharacterVariantWithItem($input: ChangeCharacterVariantWithItemInput!) {
  changeCharacterVariantWithItem(input: $input) {
    ...
    `;
export type ChangeCharacterVariantWithItemMutationFn = Apollo.MutationFunction<ChangeCharacterVariantWithItemMutation, ChangeCharacterVariantWithItemMutationVariables>;

/**
 * __useChangeCharacterVariantWithItemMutation__
 *
 * To run a mutation, you first call `useChangeCharacterVariantWithItemMutation` within a React component and pass it any options that fit your needs.
 * When your component renders, `useChangeCharacterVariantWithItemMutation` returns a tuple that includes:
 * - A mutate function that you can call at any time to execute the mutation
 * - An object with fields that represent the current status of the mutation's execution
 *
 * @param baseOptions options that will be passed into the mutation, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options-2;
 *
 * @example
 * const [changeCharacterVariantWithItemMutation, { data, loading, error }] = useChangeCharacterVariantWithItemMutation({
 *   variables: {
 *      input: // value for 'input'
 *   },
 * });
 */
export function useChangeCharacterVariantWithItemMutation(baseOptions?: Apollo.MutationHookOptions<ChangeCharacterVariantWithItemMutation, ChangeCharacterVariantWithItemMutationVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useMutation<ChangeCharacterVariantWithItemMutation, ChangeCharacterVariantWithItemMutationVariables>(ChangeCharacterVariantWithItemDocument, options);
      }
export type ChangeCharacterVariantWithItemMutationHookResult = ReturnType<typeof useChangeCharacterVariantWithItemMutation>;
export type ChangeCharacterVariantWithItemMutationResult = Apollo.MutationResult<ChangeCharacterVariantWithItemMutation>;
export type ChangeCharacterVariantWithItemMutationOptions = Apollo.BaseMutationOptions<ChangeCharacterVariantWithItemMutation, ChangeCharacterVariantWithItemMutationVariables>;
...
/**
 * __useGetCommunityShopPurchasesQuery__
 *
 * To run a query within a React component, call `useGetCommunityShopPurchasesQuery` and pass it any options that fit your needs.
 * When your component renders, `useGetCommunityShopPurchasesQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useGetCommunityShopPurchasesQuery({
 *   variables: {
 *      communityId: // value for 'communityId'
 ...
 * });
 */
export function useGetCommunityShopPurchasesQuery(baseOptions: Apollo.QueryHookOptions<GetCommunityShopPurchasesQuery, GetCommunityShopPurchasesQueryVariables> & ({ variables: GetCommunityShopPurchasesQueryVariables; skip?: boolean; } | { skip: boolean; }) ) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<GetCommunityShopPurchasesQuery, GetCommunityShopPurchasesQueryVariables>(GetCommunityShopPurchasesDocument, options);
      }
export function useGetCommunityShopPurchasesLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<GetCommunityShopPurchasesQuery, GetCommunityShopPurchasesQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<GetCommunityShopPurchasesQuery, GetCommunityShopPurchasesQueryVariables>(GetCommunityShopPurchasesDocument, options);
        }
export function useGetCommunityShopPurchasesSuspenseQuery(baseOptions?: Apollo.SkipToken | Apollo.SuspenseQueryHookOptions<GetCommunityShopPurchasesQuery, GetCommunityShopPurchasesQueryVariables>) {
          const options = baseOptions === Apollo.skipToken ? baseOptions : {...defaultOptions, ...baseOptions}
          return Apollo.useSuspenseQuery<GetCommunityShopPurchasesQuery, GetCommunityShopPurchasesQueryVariables>(GetCommunityShopPurchasesDocument, options);
        }
export type GetCommunityShopPurchasesQueryHookResult = ReturnType<typeof useGetCommunityShopPurchasesQuery>;
export type GetCommunityShopPurchasesLazyQueryHookResult = ReturnType<typeof useGetCommunityShopPurchasesLazyQuery>;
export type GetCommunityShopPurchasesSuspenseQueryHookResult = ReturnType<typeof useGetCommunityShopPurchasesSuspenseQuery>;
export type GetCommunityShopPurchasesQueryResult = Apollo.QueryResult<GetCommunityShopPurchasesQuery, GetCommunityShopPurchasesQueryVariables>;
...
export const TradeComposerDocument = gql`
  ...
  myCharacters: characters(
    filters: {ownerId: $meId, communityId: $communityId, isTradeable: true, limit: 100}
  ) {
    characters {
      id
      name
  ...
  theirCharacters: characters(
    filters: {ownerId: $themId, communityId: $communityId, isTradeable: true, limit: 100}
  ) {
    characters {
      id
      name
