...
- **Member discovery specs**: the members list, the search box's `@` mode and its per-member drill-down, and the trade composer, each reaching another member's inventory, plus the community member profile they now route through. (#349)
...
- **The suite runs on subdomains, from `e2e.localhost`**: every community URL a preset hands out is now absolute on its own host, personas are seeded with the `chardb_rt` refresh cookie instead of two localStorage tokens, and `tests/navigation/subdomain-routing.e2e.ts` covers the host split, the legacy redirects and one sign-in reaching a community host. (#339)
