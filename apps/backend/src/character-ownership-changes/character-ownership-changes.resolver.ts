...
export class CharacterOwnershipChangesResolver {
  ...
  async createCharacterOwnershipChange(
    ...
    const prismaResult = await this.characterOwnershipChangesService.create(serviceInput);
  ...
  async findAll(
    ...
    const serviceResult = await this.characterOwnershipChangesService.findAll({ first, after });
  ...
  async findByCharacter(
    ...
    const serviceResult = await this.characterOwnershipChangesService.findByCharacter({ characterId, first, after });
    return mapPrismaCharacterOwnershipChangeConnectionToGraphQL(serviceResult);
  }
  ...
  async removeCharacterOwnershipChange(
    ...
    return { removed: true, message: 'Character ownership change successfully deleted' };
  }

  // Field resolvers for relations
  @ResolveField('character', () => Character)
  async resolveCharacter(@Parent() ownershipChange: CharacterOwnershipChange): Promise<Character> {
    const prismaCharacter = await this.charactersService.findOne(ownershipChange.characterId);
    return mapPrismaCharacterToGraphQL(prismaCharacter);
  }

  @ResolveField('fromUser', () => User, { nullable: true })
  async resolveFromUser(@Parent() ownershipChange: CharacterOwnershipChange): Promise<User | null> {
    if (!ownershipChange.fromUserId) return null;
    
    const prismaUser = await this.usersService.findById(ownershipChange.fromUserId);
    if (!prismaUser) {
      throw new NotFoundException(`User with ID ${ownershipChange.fromUserId} not found`);
    }
    return mapPrismaUserToGraphQL(prismaUser);
  }

  @ResolveField('toUser', () => User)
  async resolveToUser(@Parent() ownershipChange: CharacterOwnershipChange): Promise<User> {
    const prismaUser = await this.usersService.findById(ownershipChange.toUserId);
    if (!prismaUser) {
      throw new NotFoundException(`User with ID ${ownershipChange.toUserId} not found`);
    }
    return mapPrismaUserToGraphQL(prismaUser);
  }
}