...
export class TraitReviewResolver {
  ...
  async traitReviewQueue(
    ...
    const result = await this.traitReviewService.getQueueForCommunity(
      communityId,
      filters,
      first,
      offset,
  ...
  async characterTraitReview(
    ...
    const review =
      await this.traitReviewService.getActiveReviewForCharacter(characterId);
  ...
  async approveTraitReview(
    ...
    const review = await this.traitReviewService.approveReview(
      input.reviewId,
      user.id,
    );
    return mapPrismaTraitReviewToGraphQL(review);
