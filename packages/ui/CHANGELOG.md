...
- **`Avatar`**: a user's picture as a circle, or their initials when there is
  none, falling back to initials when the image fails to load.

### Fixed

- `UserTypeahead`'s debounced search no longer leaves a pending search to fire
  after unmount.
