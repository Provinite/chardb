...
export const extractArtistsCommand: CommandModule<object, ExtractArtistsArgs> =
  {
    ...
    handler: async (argv) => {
      ...
      if (!(await fileExists(manifestPath))) {
        logger.error("Image manifest not found. Run download-images first.");
        process.exit(1);
      }
      ...
      logger.info(
        `Loaded ${firstArtistByNumericId.size} firstArtist entries from parsed characters`,
      );

      const noMatchTitles: Array<{
        numericId: string;
        field: string;
        title: string;
      }> = [];
