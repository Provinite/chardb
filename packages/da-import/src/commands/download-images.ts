...
function makeFailedDownload(sourceUrl: string, error: string): ImageDownload {
  return {
    sourceUrl,
    resolvedUrl: "",
    localPath: "",
      ...
      for (let i = 0; i < toProcess.length; i++) {
        ...
        if (!char.url) {
          ...
          try {
            ...
            logger.warn(
              `Original failed: ${char.name} (${char.numericId}): ${msg}`,
            );
        ...
        if (!char.currentRefUrl) {
          ...
          try {
            ...
            currentRef =
              existing?.currentRef ??
              makeFailedDownload(char.currentRefUrl, msg);
            refFailed++;
          }
        }
